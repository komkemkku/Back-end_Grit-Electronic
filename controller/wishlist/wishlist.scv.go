...
func ListWishlistsService(ctx context.Context, req requests.WishlistsRequest) ([]response.WishlistResponses, int, error) {
	...
	query := db.NewSelect().
		...
		ColumnExpr("w.id AS product__id").
		ColumnExpr("w.name AS product__name").
		Join("LEFT JOIN products AS w ON w.id = w.product_id")
...
func GetByIdWishlistsService(ctx context.Context, id int64) (*response.WishlistResponses, error) {
	...
	err = db.NewSelect().TableExpr("wishlists AS w").
		...
		ColumnExpr("w.id AS product__id").
		ColumnExpr("w.name AS product__name").
		Join("LEFT JOIN products AS w ON w.id = w.product_id").Where("w.id = ?", id).Scan(ctx, wish)
	if err != nil {
		return nil, err
	}
	return wish, nil
